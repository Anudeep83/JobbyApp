...
class JobsDetails extends Component {
  ...
  jobsData = async () => {
    ...
    const employmentTypes = EmployList.join(',')

    const fetchingJobs = `https://apis.ccbp.in/jobs?employment_type=${employmentTypes}&minimum_package=${RadioInput}&search=${inputValue}`
    const optionsJobs = {
      method: 'GET',
      headers: {
